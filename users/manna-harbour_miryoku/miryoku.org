...
The Mouse Buttons Overlay provides mouse buttons, mods, and clipboard keys for
use with integrated or external pointing devices, used with either hand.  The
layer is available for automatic activation depending on keyboard hardware and
configuration.  It can also be activated manually by holding a bottom row pinkie
key (in which case GUI and Redo are unavailable).
